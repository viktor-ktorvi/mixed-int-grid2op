...
def polar(mag: sp.Expr, angle: sp.Expr) -> sp.Expr:
    return mag * sp.exp(sp.I * angle)


def cartesian(real: sp.Expr, imag: sp.Expr) -> sp.Expr:
    return real + sp.I * imag


def to_numpy_equation(expression: sp.Expr, printer: NumPyPrinter) -> str:
    return str(printer.doprint(expression)).replace("numpy", "np")


def get_real_imag(expression: sp.Expr) -> tuple[sp.Expr, sp.Expr]:
